...
class HassApiClient:
    ...
    def get_state_history(self, entity_ids, start_time=None, end_time=None):
        ...
        #print(url)
        response = self.get_data(f"/api/history/period/{start_time_str}", 
                                 filter_entity_id=",".join(entity_ids))
        ...
        history_df['last_updated'] = pd.to_datetime(history_df['last_updated'], format='ISO8601')
        history_df['last_changed'] = pd.to_datetime(history_df['last_changed'], format='ISO8601')
        if self.tz is not None:
                history_df['last_updated'] = history_df['last_updated'].dt.tz_convert(self.tz)
                history_df['last_changed'] = history_df['last_changed'].dt.tz_convert(self.tz)
        return history_df
